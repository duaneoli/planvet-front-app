...
const Input: React.FC<InputProps> = ({ label, icon, error, className = "", ...props }) => {
  return (
    <div className="space-y-1 w-full">
      {label && (
        <label className="block text-xs font-bold text-slate-400 uppercase tracking-tight ml-1">
          {label}
        </label>
      )}
      <div className="relative">
        ...
        <input
          className={`
            ...
            ${error ? "border-rose-300 focus:ring-rose-500" : "border-slate-200 focus:ring-azul-500"}
